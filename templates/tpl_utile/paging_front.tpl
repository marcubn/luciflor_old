...
{else}
	<div class="pagination">
		...
		{else}
			<a href="{if $pgAction!=""}/{$pgAction}{/if}/pagina-{math equation="x-1" x=$pgNo}/" class="prev">Pagina anterioara</a>
		{/if}
		<div class="pages">
			{foreach from=$pages item=page key=key}
				...
				{else}
					<a href="{if $pgAction!=""}/{$pgAction}{/if}/pagina-{$page}/" title="Pagina {$page}">{$page}</a>
				{/if}
			{/foreach}
		</div>
		{if $pgNo==$noPages}
		...
		{else}
			<a href="{if $pgAction!=""}/{$pgAction}{/if}/pagina-{math equation="x+1" x=$pgNo}/" class="next">Pagina urmatoare</a>
		{/if}
