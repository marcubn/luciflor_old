    	...
    	{if $recList|@count>0}
            {foreach from=$recList item=item key=key}
                {if $item.media_type=='video_embed'}
                    <li class="span1" style="float: left; width:150px; height:180px; margin-bottom:10px;{if $item.def==1}border-radius: 3px;{/if}">
                        <table class="ctrls" id="ctrl_{$item.$idName}" style="width:140px; position: absolute; display:none;">
                          <tr>
                              <td>
                                <button type="button" onclick="window.location='index.php?obj={$smarty.get.obj}&action=change&{$idName}={$item.$idName}&fieldName={$flagName}{$paging_options}'" style="width: 35px;" class="btn btn-default" title="Switch status"><i class="glyphicon glyphicon-{if $item.$flagName==1}lock{else}ok{/if}">&nbsp;</i></button>
                              </td>
                              <td align="center">
                        ...
                        <a onmouseover="$('.ctrls').hide(); $('#ctrl_{$item.$idName}').show(); " href="javascript:popup('index.php?obj=uplphoto&action=video_preview&video={if $item.video_src=='youtube'}http://www.youtube.com/v/{$item.video_code}{elseif $item.video_src=='vimeo'}http://player.vimeo.com/video/{$item.video_code}{/if}&type={$item.video_src}', 550, 350, 'uplimg', 1); void(false);" class="thumbnail" title="Click to preview" style="height:180px; width:140px; overflow:hidden;">
                        <img src="/img/play.png" width="50" height="50" style="margin-top: 55px;"/>   
                        </a>
                  </li>
                {else}
                    <li class="span1" style="float: left; width:150px; height:180px; margin-bottom:10px;{if $item.def==1}border-radius: 3px;{/if}">
                        ...
                        <a class="thumbnail fancybox" style="height:180px; width:140px; overflow:hidden;" onmouseover="$('.ctrls').hide(); $('#ctrl_{$item.$idName}').show(); " href="{$smarty.const.PHOTOS_UPLOAD_URL}/{$item.file}" title="Click to preview">
