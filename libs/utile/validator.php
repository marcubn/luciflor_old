...
class validator {
	...
	function validator($requestArray) {
		...
		$this->validatorId = $id;
		$this->request = ($requestArray) ? $requestArray : $_POST;
	}
	...
	function value($field, $operator, $length) {
		switch($operator) {
			...
			case "!=":
				...
				break;
			case "md5":			
				if( md5($this->request[$field]) == md5($length)) {
					return true;
				} else {
					$this->setError($field, 106);
					return false;
	...
	function phone($field = NULL)	{
		
		if(!is_null($field))	{
			if(preg_match("/^[+]*[0-9\040]+$/",$this->request[$field]))	{
				return true;
			}	
			else	{	
	...
	function customRegEx($field, $regEx) {
        if( $regEx == '' ) { return false; }
        $ret = preg_match($regEx, $this->request[$field]);
        if( !$ret ) {
	...
	function setError($field, $error) {
		...
		$this->errorStack[] = array('code' => $error, 'field' => $field, 'message' => $this->errorMessages[$field]);
	}
	...
	function pushError($field, $message) {
		...
		$this->errorStack[] = array('code' => -1, 'field' => $field, 'message' => $message);
		$this->valid = false;
	}
	...
	function getErrorStack() { return $this->errorStack; }
	
	function getFieldError($field) { return $this->errors[$field]; }
	...
	function getField($field) { return $this->request[$field]; }
    
	function customValidationRule( $field, $rule, $ruleParams = array() ) {
		if (!is_callable( $rule )) {
			return false;    
		}
		array_unshift( $ruleParams, $this->request[$field] );
		$ret = call_user_func_array( $rule, $ruleParams );
		if (!$ret) {
			$this->setError($field, 23);    
		}
		return $ret;
	}
/*
